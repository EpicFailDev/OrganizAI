...
export interface CreateAppOptions {
  /** Desativa o log de requisições (usado nos testes). */
  disableLogger?: boolean;
}

/**
 * Constrói a aplicação Hono com todos os middlewares e rotas.
 *
 * Foi extraído de index.ts para permitir testes de integração (via
...
export function createApp(options: CreateAppOptions = {}): OpenAPIHono<AppEnv> {
  ...
  // corpo e CORS.
  if (!options.disableLogger) app.use('*', logger());
  ...
  // Rate limit em memória (por IP) para as rotas de API /v1/* e para o /mcp.
  app.use('/v1/*', rateLimit(config.rateLimit));
  app.use('/mcp', rateLimit(config.rateLimit));
